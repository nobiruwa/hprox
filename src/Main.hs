...
import           System.PosixCompat.User        ( UserEntry(..)
                                                , getUserEntryForName
                                                , setUserID
                                                )
...
splitBy :: Eq a => a -> [a] -> [[a]]
splitBy _ [] = [[]]
splitBy c (x : xs) | c == x    = [] : splitBy c xs
...
parser = info (helper <*> ver <*> opts) (fullDesc <> progDesc desc)
 where
  ...
  bind = optional $ fromString <$> strOption
    (long "bind" <> short 'b' <> metavar "bind_ip" <> help
      "ip address to bind on (default: all interfaces)"
    )

  port = optional $ option
    auto
    (long "port" <> short 'p' <> metavar "port" <> help
      "port number (default 3000)"
    )

  ssl = many $ option
    (eitherReader parseSSL)
    (  long "tls"
    <> short 's'
  ...
  user = optional $ strOption
    (long "user" <> short 'u' <> metavar "nobody" <> help
  ...
  rev = optional $ strOption
    (long "rev" <> metavar "remote-host:port" <> help
...
main = do
  ...
  let isSSL                      = not (null certfiles)
      ...
      settings = setNoParsePath True $ setServerName "Apache" $ maybe
        id
        (setBeforeMainLoop . setuid)
        _user
        defaultSettings
  ...
  let pset = ProxySettings pauth Nothing (BS8.pack <$> _ws) (BS8.pack <$> _rev)
      proxy =
        (if isSSL then forceSSL pset else id)
          ...
          $ reverseProxy pset manager dumbApp
      port = _port
